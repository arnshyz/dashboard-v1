...
function inRange(dateStr, from, to) {
  ...
  if (to) { const end = new Date(to); end.setHours(23,59,59,999); if (t > end) return false; }
  return true;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (channel) { const set = new Set(String(channel).split(',')); rows = rows.filter(r => set.has(r.channel)); }
    if (marketplace) { const set = new Set(String(marketplace).split(',')); rows = rows.filter(r => set.has(r.marketplace)); }
    ...
    if (format === 'pdf') {
      ...
      const headers = Object.keys(rows[0]);
      doc.fontSize(9);
      const colW = Math.max(40, Math.floor((595-72)/headers.length));
      let x0 = 36, y = 90;
      headers.forEach((h,i)=>{ doc.text(h, x0 + i*colW, y, { width: colW }); });
      y += 14;
      rows.slice(0, 500).forEach(r => {
        headers.forEach((h,i)=>{ doc.text(String(r[h] ?? ''), x0 + i*colW, y, { width: colW }); });
        y += 12; if (y > 800) { doc.addPage(); y = 60; }
      });
      doc.end(); return;
    }
