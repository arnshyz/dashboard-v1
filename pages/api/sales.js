...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    if (!isAdmin) return res.status(401).json({ error: 'Unauthorized: admin key required' });
    try {
      const { rows } = req.body || {};
      if (!Array.isArray(rows) || !rows.length) return res.status(400).json({ error: 'Body must include { rows: [...] }' });
      const sheets = getSheetsClient();
      const range = `${tab}!A2`;
